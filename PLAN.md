# LinguaMCP Engram Integration Plan

> **Purpose:** Extend LinguaMCP with an optional evidence-based learning and spaced-retrieval layer inspired by [nagisanzenin/engram](https://github.com/nagisanzenin/engram), while preserving LinguaMCP's existing principle: **the connected model teaches; LinguaMCP stores, validates, schedules, and returns learner state.**
>
...
- Do not copy the reference repository wholesale. Port its theoretical invariants into LinguaMCP's MCP/file architecture.
...
LinguaMCP will maintain three distinct forms of state.
...
| LinguaMCP storage layer | Validate, persist, archive, return bounded records | Invent curricula or interpret free text |
...
- [ ] Add a feature flag/configuration so existing users can run LinguaMCP without Engram tools or context.
...
- [ ] Define which reference principles are mandatory for LinguaMCP and which remain experiments.
...
- [ ] Use UUIDs generated by LinguaMCP for IDs; callers cannot choose filesystem names.
...
- [ ] All previous LinguaMCP tests still pass.
...
Because LinguaMCP can connect to small local models:
...
- [ ] Update README product description without implying that LinguaMCP itself teaches.
